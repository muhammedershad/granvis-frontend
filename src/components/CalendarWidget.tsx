      ...
      <div className="relative">
        ...
        <div className="space-y-3 mb-6">
          {meetings.map((meeting) => (
            <div key={meeting.id} className="flex items-center space-x-4 p-4 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-300">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white/90 truncate">{meeting.title}</p>
        ...
        <div className="border-t border-white/10 pt-4">
          <h4 className="text-white/90 mb-4">Upcoming Deadlines</h4>
          <div className="space-y-3">
            {upcomingDeadlines.map((deadline, index) => (
              <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/10">
                <div>
                  <p className="text-sm text-white/90">{deadline.project}</p>
                  <p className="text-xs text-white/60">Due in {deadline.deadline}</p>
                </div>
                <Badge 
                  variant="outline" 
                  className={`${
                    deadline.priority === 'high' ? 'bg-red-500/20 text-red-300 border-red-500/30' :
                    deadline.priority === 'medium' ? 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30' :
