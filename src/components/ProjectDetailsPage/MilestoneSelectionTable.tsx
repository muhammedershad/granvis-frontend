...
export function MilestoneSelectionTable({
  ...
  const summary = milestones.reduce(
    (acc, milestone) => {
      if (milestone.invoicingStatus === "not_invoiced") {
        acc.notInvoiced++;
      }
      if (milestone.invoicingStatus === "partially_invoiced") {
        acc.partiallyInvoiced++;
      }
      if (milestone.invoicingStatus === "fully_invoiced") {
        acc.fullyInvoiced++;
      }
      return acc;
    },
                  ...
                  >
                    <TableCell>
                      <button
                        type="button"
                        onClick={() =>
                          isSelected && onToggleExpand(milestone.id)
                        }
                        className={`${
                          isSelected
                    ...
                    <TableCell>
                      <Checkbox
                        checked={isSelected}
                        onCheckedChange={() =>
                          onToggleMilestone(milestone.id, milestone)
                        }
                      />
                    </TableCell>
                      ...
                      <TableCell colSpan={7}>
                        <div className="p-4 space-y-4 border-l-4 border-blue-500">
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center h-9 px-3 bg-muted rounded-md border">
                                <span className="text-sm font-medium text-muted-foreground">
                                  {formatCurrency(
                                    milestoneItem.calculatedAmount
                                  )}
                                </span>
                              </div>
                            </div>
                          ...
                          <div className="max-w-xs space-y-2">
                            <Label
                              htmlFor={`editableAmount-${milestone.id}`}
                            ...
                            <p className="text-xs text-muted-foreground">
                              This amount will be used in the invoice. You can
                            ...
                            <div className="flex items-start gap-2 p-3 bg-orange-50 dark:bg-orange-950/20 rounded-lg border border-orange-200 dark:border-orange-800">
                              ...
                              <div className="text-sm text-orange-800 dark:text-orange-200">
                                ...
                                <p className="text-xs mt-1">
                                  This milestone has been fully invoiced (
                                  {formatCurrency(milestone.totalBilled)}).
                                  Creating another invoice may result in
                                  overbilling.
                                </p>
                              </div>
                            </div>
